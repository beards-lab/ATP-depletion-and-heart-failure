...
| How should the objective's feature weights be set? | `w = λ·R/s²` — `s` = measured between-prep spread at 8 mM, `R` = 1+0.5·min(z,4) ATP-relevance bonus, one global λ for cost-neutrality. Turns every term into a **χ²**: 1.0 = one preparation-width from the data | [FeatureWeighting](FeatureWeighting/conclusions.md) |
| How far is the model from the data, in noise units? | **14 of 16 data terms are at or below the between-prep noise floor** (χ² 0.3–3.7). Only `rsK` (χ² 33) and the unscored `rsT0` (χ² 36) are genuinely broken | [FeatureWeighting](FeatureWeighting/conclusions.md) §6 |
| Does low ATP reshape the force trace? | **No — it scales it and slows it.** All six force-*shape* features are flat (z 0.07–0.21); the whole ATP signal is `steady` ×1.35, `ktr` ×0.54, `rsK` ×0.35, `rsT0` ×0.43 | [FeatureWeighting](FeatureWeighting/conclusions.md) §3 |
...
| How many more sessions do I need, and in what order? | **6**, alternating `8-4-2-8` / `2-8-2`, **a bracket in every one**, sex balanced at every rung (stop at 4 if preps are scarce) | [ProtocolDesign](ProtocolDesign/conclusions.md) §8 |
| Where does 4 mM go inside a session? | **Second — `8-4-2-8`, never `8-2-4-8`.** In slot 3 its correction (18.6 %) exceeds its signal (15.8 %) and the uncorrected ratio's **sign flips** | [ProtocolDesign](ProtocolDesign/conclusions.md) §8.2 |
| Can I skip the rundown correction if the design is balanced? | **Only with `8-4-2-8` vs `2-8-2`** — that pair balances in *damage* (1.0–2.2 % residual); count-balanced `8-2`/`2-8` leaves 4.1–9.1 % | [ProtocolDesign](ProtocolDesign/conclusions.md) §8.3 |
...
| Why is post-restretch the *fastest* window in the model? | The SRX brake **auto-disengages under load**: entry ∝ e^(−F/σ₂) and exit ∝ e^(+F/σ_srd1) point the same way, so the in/out ratio swings **60×** (16.6 at F=0 → 0.28 at 0.87 F_iso). The rate is therefore an increasing function of the force it redevelops at — and post-restretch is the only recovery that runs at high force | [RestretchSRXRecoil](RestretchSRXRecoil/conclusions.md) §4 |
| Can detachment recoil into SRX to slow it? | **No — it goes the wrong way.** SRX is a *fast lane* at restretch force (exit 238 s⁻¹ vs `kah` 102), so routing detachment there *speeds* the recovery and refills `PD`. And the return path has no authority anyway: `PD` = 0.64 is a reservoir 4× the bound mass | [RestretchSRXRecoil](RestretchSRXRecoil/conclusions.md) |
| Would a fixed *dwell* in SRX (constant outflow, not a rate) fix it? | **No.** On the shared pool it brakes post-slack instead (`ktr` 1.07 → 0.63 — the pool fills 370× more there); on a pool fed only by rupture it is worth **0.01** in `rsK` vs a first-order exit. Nothing on the return path can work | [RestretchSRXRecoil](RestretchSRXRecoil/conclusions.md) §9 |
| **What *does* slow the post-restretch recovery?** | A **load-engaged availability deficit** (compliant realignment): `ka` gated by bound mass stripped *while the filament stays loaded*. `rsK` **×2.35 → ×1.12** with `ktr` ×1.04 and isometric force unchanged to the digit — reached **without `k2`** and so without its force/`ktr` couplings. Bill is restretch shape, partly payable by `kA2re` | [RestretchSRXRecoil](RestretchSRXRecoil/conclusions.md) §8 |
| Does it survive a refit? | A 3-round (1.3 h, not converged) joint refit **keeps the mechanism** (`k_cr` 6 → 1.56, never walked to 0) at `rsK` ×1.70, `ktr` ×1.03, `vall_y` near-exact. Full objective 6.51 → 10.96 = worse; but **`ovrsht_dy` alone is 161 % of the regression**, and that feature is already on SlackDataAnalysis's "do not fit" list (CV 115 %, sign flips). Without it: **5.30 → 3.76** | [RestretchSRXRecoil](RestretchSRXRecoil/conclusions.md) §11 |
| Were the earlier ktr-window fixes ever applied? | **No.** `UseMaxwellTensionOnly = 1` (called *"strictly an improvement — take it before anything else"*) is still `0`, and `tau_reg` is still 27 ms against a recommended 1–5 ms. Both drifted back because the current objective sets `RunKtr = 0` | [RestretchSRXRecoil](RestretchSRXRecoil/conclusions.md) §10 |
...
| **FeatureWeighting** | findings | Replaces the hand-set `fn` weights with `λ·R/s²` from the **measured between-preparation spread** at 8 mM (3 preps, Baker excluded), tilted toward features that move with ATP. Cost-neutral at `realign4_opt` (3.8513). Yields the χ² diagnostic: **only the re-stretch recovery is still broken**; `peak1_dSL` (76 % spread) was over-weighted 5×, FV over-weighted 25×. Ships `params/spreadw_seed.m` + `RunOptimSpreadW.m`. | [conclusions](FeatureWeighting/conclusions.md) |
...
| **RestretchSRXRecoil** | **falsified** | Tests whether the strong-bridge detachment can recoil into **SRX** instead of DRX — either for the whole cycle (`SRXFromR2`) or for the high-strain rupture branch only (`SRXFromR2HighStrain`), so the SRX jump exists only during a restretch. Both wired default-OFF and exactly inert (ΔL2 = 0). **All 12 variants move `rsK` the wrong way** (×2.35 → 2.24–2.82) and none beats the baseline objective. The cause is measured: the SRX exit is force-**accelerated**, so the detour is a **shortcut** at the restretch force (4.2 ms vs the normal 9.8 ms) and *raises* `PD`. Flattening that gate is the only sign-flipping lever and costs F_iso 76 → 60 kPa. Yields three extensions to the earlier work: "an obligatory in-series step" only clamps if it sits at the **bottleneck** (the return path is a reservoir, not a bottleneck); the SRX brake **auto-disengages under load**, which structurally obliges the model to make post-restretch its fastest window; and `vall_y`/`rsK` are **one lever with opposite signs** (r = −0.993). The force-flat-exit control flips the sign, confirming the diagnosis and pointing at a load-engaged availability deficit as the mechanism class that can work. **Part 2 builds it** (`UseLoadRealign`: `ka` gated by bound mass stripped while loaded, one scalar state, steady-state-inert by construction) — and it **works**: `rsK` spans ×2.43 → ×0.76 with `ktr` 1.06–1.07 and `steady` 76.2 kPa across the whole range, landing at **×1.12** (`rsK`/cycle 48.7/48.0/48.1/49.2/50.6 vs data 46.3/37.8/41.6/42.9/50.0) with `kA2re`+`eta_M` compensation. The load-blind control proves the force weighting is the selectivity (same `rsK` gain, but `ktr` → 0.83). Bill is restretch shape (L2 61.6); `kA2re` pays half of it and no refit has been run. Part 2 also **falsifies the fixed-dwell/constant-outflow idea** in both placements. | [conclusions](RestretchSRXRecoil/conclusions.md) |
| **ProtocolDesign** | findings | Costs the *next* dataset: order, repeats, sample size, sex, and a 4 mM arm. **Plan: `8-4-2-8` alternating with `2-8-2`, a bracket in every session, 6 preps / 21 activations, sex balanced at every rung** (stop after S4 = 4 preps if scarce). Two results drive it. **(1)** A bracket in every session measures φ per prep instead of transferring it — which removes the objection to stacking three doses, since a *measured* 12.5 % correction beats an *assumed* 5.1 % one. **(2)** Slot order then matters more, not less: in `8-2-4-8` the correction on 4 mM (18.6 %) **exceeds its own signal (15.8 %)** and the uncorrected ratio reads **0.943 — the sign flips**, 4 mM appearing weaker than 8 mM when it is ~16 % stronger. The smallest-signal dose takes the earliest slot. **Bonus: `8-4-2-8` vs `2-8-2` balances in *damage*, not just count** (−12.5 % vs +16.0 %), so a naive uncorrected 1:1 average leaves only **1.0–2.2 % across the whole φ range** against 4.1–9.1 % for the plain `8-2`/`2-8` balance strategy.md §2.4 rejected — the correction becomes a *check*, and corrected-vs-uncorrected disagreement >2 % is itself the diagnostic. **4 mM is the geometric mean of 2 and 8** so the dose laws separate by only 3.9–5.8 %: it is a model-validation point (89 % power that it differs from 8 mM at n=3), **not** a dose-response study (curvature needs ~42 preps). Sex is nowhere a variable in this repo — balance it (free; guards a live sex↔order aliasing risk), don't power for it. | [conclusions](ProtocolDesign/conclusions.md) |

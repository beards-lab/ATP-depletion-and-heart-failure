...
if ~exist('CASES','var') || isempty(CASES)
    CASES = { 'baseline',        'params/rskR2_w025_opt.m', [0.00 0.45 0.74]
...
R = struct('name',{},'t',{},'F',{},'feat',{},'ct2',{},'L2',{},'pas',{},'col',{});
for c = 1:size(CASES,1)
    ...
    if c == 1     % build the objective ONCE, from the baseline, and reuse it
        fn = params0.fn;
...
for i=1:numel(R); plot(R(i).t-t0, R(i).F, 'Color',R(i).col,'LineWidth',LW,'DisplayName',lab{i}); end
...
for i=1:numel(R)
    ...
    plot((R(i).t(mm)-vt(iS(1),1))*1000, R(i).F(mm),'Color',R(i).col,'LineWidth',LW,'DisplayName',R(i).name);
end
yl = ylim;
...
for i=1:numel(R)
    plot(R(i).feat.FV_v, R(i).feat.FV_fnorm,'o-','Color',R(i).col,'MarkerSize',9, ...
...
for i=1:numel(R)
    plot(v, R(i).feat.FV_normpowerAvg,'o-','Color',R(i).col,'MarkerSize',10, ...
...
fprintf('\nsaved fin_slack_full / fin_slack_cycle1 / fin_fv / fin_pv .png\n');

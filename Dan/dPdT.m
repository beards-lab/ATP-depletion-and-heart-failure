function g = dPdT(~,x,N,V,del_U)

alphaL = 2000; % length-dependent unfolding rate
...
g(1,:)   = (alphaL*L.^6 + alpha0)*P0;
g(2:N,:) = (alphaL*(max(0,L - (1:N-1)'*del_U ).^6) + alpha0 ).*P(1:N-1) ;
g(1:N-1) = g(1:N-1) - (alphaL*(max(0,L - (1:N-1)'*del_U ).^6) + alpha0).*P(1:N-1) ;

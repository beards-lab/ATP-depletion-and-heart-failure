...
% Fs = kS*max(0,L-s).^nS ;
deltaF = (kS*max(0,L-s).^nS - Fc);
% Vd = -deltaF/mu; % distal chain strain rate
% Vp = V - Vd; % proximal chain strain rate
Vp = deltaF/mu; % proximal chain strain rate
...
g(ij(1,:)) = g(ij(1,:)) - (1/ds)*(pu(1,:).*Vp(1,:))';
g(ij(2:Nx,:)) = g(ij(2:Nx,:)) - (1/ds)*pu(ij(2:Nx,:)).*max(0,Vp(2:end,:)) + ...
                (1/ds)*pu(ij(2:Nx,:)-1).*max(0,Vp(1:end-1,:));  

...
#### 1.12.1 Velocity-Dependent Cross-Bridge Distortion Reset (Land 2017)

**Physical motivation.** During shortening, strongly bound P2 heads accumulate at negative strain (compressed state) as the sarcomere slides. These heads resist shortening (negative force generation). In the Land (2017) model, the transition from the weakly bound (W) to the strongly bound (S) state resets the cross-bridge distortion to zero, effectively preventing accumulation of negative-strain strongly bound heads. This is equivalent to saying that the power stroke erases the pre-stroke deformation. This mechanism is directly responsible for the shape of the FV curve at high shortening velocities.

**Relevant protocols.** FV curve (especially curvature and Vmax), ktr rise rate (time scale of force redevelopment).

**Literature support.** Land et al. (2017) demonstrate that the distortion reset is required to reproduce the human cardiac FV curve. The concept is consistent with single-molecule observations that the power stroke occurs at the moment of strong binding (Capitanio et al. 2006; Guilford et al. 1997). Without this reset, the FV curve becomes too curved (low Vmax, high drag at intermediate velocities).

**Suggested implementation.** When the P1→P2 transition occurs at strain s, deposit the P2 probability at strain `s + dr` (the power stroke distance) rather than at s. This is a redistribution in the probability flux `R12`. The model already computes `dr` (power stroke = 0.01 μm = 10 nm), but the current R12 flux moves probability in-place (same strain bin); implementing the shift requires redistributing the leaving flux of R12 to `s + dr` bins, not to the same s. This is equivalent to the "distortion reset" of Land (2017).
...
**Physical motivation.** During sarcomere stretch (as in the restretch phase of the slack-restretch protocol), cross-bridges are under positive strain and detach more slowly. This "catch bond" behavior (Guo and Bhattacharya 2012; Nishizaka et al. 1995) means that force rises faster during restretch than predicted by a simple model where all rate constants are symmetric. Additionally, cooperative effects between neighboring cross-bridges on the same thin filament regulatory unit (nebulin, troponin-spanning) can cause burst-like activation during restretch.

**Relevant protocols.** Slack-restretch force rise slope, early overshoot in force transient.

**Literature support.** Brenner (1988), Tewari (as cited), Campbell (2018).

**Suggested implementation.** Increase the positive-strain asymmetry of kd(s): reduce detachment rate exponentially for s > 0 (i.e., a catch bond term in R1D). The current PCHIP multiplier for R1D already does this to some extent; the issue is whether the magnitude is sufficient.
...
| Distortion reset at P1→P2 transition (Land 2017) | Not implemented | Required to reproduce FV curvature at high velocities; standard in modern cardiac XB models | Land 2017; Capitanio et al. 2006 |
...
- Linari, M. et al. (2009). "Force generation by skeletal muscle is controlled by mechanosensing in myosin filaments." *Nature* 528: 276–279.

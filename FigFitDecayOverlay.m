...
w = 0.26; h = 0.85; x0 = 0.04; y0 = 0.1; gap = 0.005;
% data ranges - pca11
xrng = [-10 17]; xrng_add = [55 60]; yrng = [4e-1 18];
% data ranges - pca4
% xrng = [-10 17]; xrng_add = [55 60]; yrng = [3.3 50];

% Font size
fs = 16;

dpi = (w-gap)/(diff(xrng) + diff(xrng_add)); % 
w1 = diff(xrng)*dpi; w2 = diff(xrng_add)*dpi;

% left
% axes left main
a_lm = axes('Position', [x0 y0 w1 h], 'XLim',xrng,'XTick',floor((xrng(1):5:xrng(end))/5)*5, ... 
...
hold on; % keep the settings from overwritting
% axes left adjacent
...
% center
a_cm = axes('Position', [0.5-w/2 y0 w1 h], 'XLim',xrng,'XTick',floor((xrng(1):5:xrng(end))/5)*5, ... 
...
a_ca = axes('Position', [0.5-w/2 + w1 + gap y0 w2 h], 'Xlim', xrng_add,'XTick',floor((xrng_add(1):5:xrng_add(end))/5)*5, 'YTick', [],... 
'YLim',yrng, 'FontSize',fs, 'YScale','log', 'TickLabelInterpreter','latex'); 
hold on;
...
for i_rds = [4 3 2 1]
        ...
        if plotResults
            ...
            axes(a_lm);
            l_d(i_rds) = plot(t_s_ru + rampShift(i_rds)*0 - rds(i_rds), Fint_ru + c, '-', Color=cc, LineWidth=clw);
    ...
    pf_v = pf(t_ext);
    l_f = loglog(t_ext, pf_v, ':', Color=[0 0 0], LineWidth=4);
    ...
    text(12, c-0.5, '$T_{ss}$', 'Interpreter', 'Latex', 'FontSize',fs)

    % title('A: Linear axis plot of a_i(x-r_d - \tau_{0, i})^{\tau_i} + Fss')
    valids = isgraphics(l_d);
    legends = {'$t_r = 100s$','$t_r = 10s$','$t_r = 1s$','$t_r = 0.1s$','$T_{ss}$'}
    legend([l_d(valids) l_tss], legends([valids true]), 'Interpreter','latex', 'FontSize',fs);

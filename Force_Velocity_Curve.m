...
% plot(-vel, Force(:,1)'.*(-vel),'b-','linewidth',1.5);
% plot(-vel, Force(:,2)'.*(-vel),'g-','linewidth',1.5);
% plot(-vel, Force(:,3)'.*(-vel),'r-','linewidth',1.5);
...
figure(4); clf; axes('position',[0.15 0.15 0.8 0.8]); hold on;
plot(UNR(:,1), -vel, 'b-','linewidth',1.5);
plot(UNR(:,2), -vel, 'g-','linewidth',1.5);
plot(UNR(:,3), -vel, 'r-','linewidth',1.5);
ylabel('Velocity (ML/s)','interpreter','latex','fontsize',16);
xlabel('$U_{NR}$','interpreter','latex','fontsize',16);
set(gca,'fontsize',14); 
axis([0 1 0 6]); grid
box on;

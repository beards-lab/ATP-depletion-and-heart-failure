...
[t,PU] = ode15s(@dPUdT,[0 1],PU0,[],N,dS,MgATP,Pi,MgADP,g0);
PU = PU(end,:);
p1 = PU(1:1*N+1);
p2 = PU(1*N+2:2*N+2);
p3 = PU(2*N+3:3*N+3);
...
Force = kstiff2*p3_0 + kstiff1*(  p2_1 + p3_1 ); % initial steady-state force
...
%   SL(i+1) = SL(i) + dt*vel;
  ...
  p2_0 = dS*sum(p2); p2_1 = dS*sum(s.*p2);
  p3_0 = dS*sum(p3); p3_1 = dS*sum((s+dr).*p3);
  Force = kstiff2*p3_0 + kstiff1*( p2_1 + p3_1) ;
  forcepositive = Force > 20;
  F_sim = [F_sim, Force];

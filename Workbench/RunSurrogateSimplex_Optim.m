...
DEBUG = true;             % <<< true = ~10 min smoke test; false = grid run >>>
if DEBUG
    ...
    N_ROUNDS        = 1000;
    TIME_BUDGET_HRS = 20;   % <<< set the grid wall-clock budget here (hours) >>>
    SURR_EVALS      = 120;
    SIMPLEX_EVALS   = 120;
end
...
%% -------- base = the current best regavail snapshot ---------------------
params0 = getParams();
...
slow_sag     = {'kSE_M', 'eta_M', 'mu', ...
                'PieceWiseStrainDepR1DParams__2', 'PieceWiseStrainDepR1DParams__3'}; % vall2_dy undershoot
...
pool = unique([ktr_iron, onset_delay, slow_sag, force_scale, fv_shoulder, srx_pool], 'stable');
...
if RESUME && exist(STATE_FILE, 'file')
    ...
    state.best_cost = evaluateBakersExp(ones(1,1), setMods(params0, {}), true);
    state.round     = 0;
    state.stall     = 0;
    state.history   = [];

...
elseif ceil(s_p0) > params.ss
    ...
    s_i1 = params.ss;
    s_i0k = 0;
    % warning(sprintf('Out of bounds! At %0.6fs and SL %0.2fum, the s(end) was %0.2f', t, SL, s(end)));
else
...
if params.FudgeVmax && t > -1
    if params.kSE*LSE >= -1e3
        ...
        SLe = SL;
        velHS = -(params.FudgeA*SLe^2 +params.FudgeB*SLe + params.FudgeC);
        % velHS = -params.vmax;
    end
    dLSEdt = vel - velHS;
else
...
%         dU_SR = + 0*sum(XB_Ripped)*dS - params.ksr0*exp(F_total/params.sigma1)*P_SR + params.kmsr*exp(-F_total/params.sigma2)*PT;
    ...
    dU_SR = -RSR2PT  + RPT2SR;
    dU_SR =  - params.ksr0*exp(F_total/params.sigma1)*P_SR + params.kmsr*exp(-F_total/params.sigma2)*PT;
...
%% breakpints
if t >= 1.8420162642e+00 % || t > 0 && (p1_0 + p2_0 + PD + P_SR) > 1

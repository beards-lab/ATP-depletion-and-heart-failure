...
%             p2_0 = opts.dS*sum(p2); 
            p2_1 = params.dS*sum(params.s.*p2);
        %     p3_0 = dS*sum(p3); p3_1 = dS*sum(s.*p3);
...
%             out.t(1) = opts.T(1) + dt;
%         end
        out.NR(i) = PU(j, 3*params.N+4);
        out.NP(i) = PU(j, 3*params.N+5);
        out.SL(i) = PU(j, 3*params.N+6);

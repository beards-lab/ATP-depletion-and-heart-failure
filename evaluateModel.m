...
%     Force = zeros(size(vel));
    ...
    if vel == 0
        % Zero velocity:
        [t,PU] = ode15s(fcn,T,PU0,[],N,dS,params,g0);
    ...
    %     p3_0 = dS*sum(p3); p3_1 = dS*sum(s.*p3);
        ...
        if isfield(opts, 'ValuesInTime') && opts.ValuesInTime
            ...
            out.t = zer;
            out.SL = zer;
            % dsitriburourourroution in time
            out.p1_0 = zer;
            out.p2_0 = zer;
            out.p3_0 = zer;
            out.p1_1 = zer;
...
%             out.p3p_t = zer;
        ...
        [~,PU] = ode15s(fcn,[0 dt/2],PU0,[],N,dS,params,g0);
        PU = PU(end,:); 
        ...
        for i = 1:(Nstep-1)         
          ...
          [~,PU] = ode15s(fcn,[0 dt],PU,[],N,dS,params,g0);
          PU = PU(end,:);           
        ...
        [~,PU] = ode15s(fcn,[0 dt/2],PU,[],N,dS,params,g0);

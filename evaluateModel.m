function [Force, t, SL] = evaluateModel(fcn, vel,T,MgATP,Pi,MgADP,g0, opts)
...
%             PU = PU(end,:);
            ...
            p2_1 = dS*sum(s.*p2);p3_0 = dS*sum(p3); p3_1 = dS*sum((s+dr).*p3);       
            F(i) = kstiff2*p3_0 - max(-kstiff1*(p2_1 + p3_1), 0)^g0(20) + mu*vel;
            if i > 1
                t(i) = t(i-1) + dt;
                SL(i) = SL(i-1) +vel*dt;
            end
